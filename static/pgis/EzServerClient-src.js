...
(function (window, document, undefined) {
	...
	EzServerClient.is3DMap=false;//是否为3DMap
	...
	EzServerClient.CRS = {
		latLngToPoint: function (latlng, zoom) { // (LatLng, Number) -> Point
			var projectedPoint = this.projection.project(latlng),//中心点坐标
				scale = this.scale(zoom);//计算网格实际大小
	...
	EzServerClient.Map = EzServerClient.Class.extend({
		...
		layerPointToLatLng: function (point, zoom) { // (Point)
			if (this.options.isEzMap) {
				/**
				 * @author qianleyi
		...
		_initContainer: function (id) {
			...
			} else if (false&&container._leaflet) {
		...
		_layerAdd: function (layer) {
			layer.onAdd(this);
			//该处代码会引起一个bug：浏览器resize，再上点，会出现地图中心点飘移问题，注释掉这句代码，问题得以解决
		...
		_getNewTopLeftPoint: function (center, zoom) {
			var viewHalf = this.getSize()._divideBy(2);//中心点像素坐标
	...
	EzServerClient.TileLayer.HotSpot2 = EzServerClient.TileLayer.extend({
		...
		onAdd: function(map) {
			...
			if (!this.options.isMulti) {
				...
				this._marker = EzServerClient.marker([100, 0], {
					icon: this.options.icon
				}).addTo(this._map);
				this._marker.setOpacity(0);
		...
		_update: function() {
			...
			if (zoom > this.options.maxZoom || zoom < this.options.minZoom) {
				this._tileImage._container.children[0].innerHTML="";//解决缩放级别超过范围，还存在切片的问题
				return;
			}
		...
		_loadTile: function(tile, tilePoint) {
			...
			//modify by zhouyx
			//热点图加加载数据采用jquery-jsonp
			//取消使用xhr方式
			var hotspoturl = this.getTileUrl(this._hoturl, tilePoint);
			if(hotspoturl){
				this._jsonp(hotspoturl,this._hostJsonpCallback);
			}
		...
		_jsonp:function(url,callback){
			var layerName=this._name;
			var tietleLayer=this;
			$.getJSON(url+"&callback=?",
				function(json){
					if(json&&json.length>0){
						tietleLayer.colAndRow=tietleLayer._urlToParames(url);
					}
					callback&&callback(json,tietleLayer);
				});
		},
		_urlToParames:function(url){
			var name,value,parames={};
			var str=url; //取得整个地址栏
			var num=str.indexOf("?")
			...
			for(var i=0;i < arr.length;i++){
				num=arr[i].indexOf("=");
				if(num>0){
					parames[arr[i].substring(0,num)]=arr[i].substr(num+1);
				}
			}
			return parames["TileCol"]+":"+parames["TileRow"];
		},
		_hostJsonpCallback:function(hotspots,tietleLayer){
			var that = tietleLayer,
				colAndRow = that.colAndRow;
			...
			for (var j = 0; j < this.selectArrayOfHotSpots.length; j++) {
				...
				if (hotspotareas && tileID in hotspotareas) {
					var hotspotArr = hotspotareas[tileID],
						len = hotspotArr.length;
					for (var i = 0; i < len; i++) {
						var hotspotlatlng = EzServerClient.latLng(hotspotArr[i].Y||hotspotArr[i].y, hotspotArr[i].X||hotspotArr[i].x),
							hotspotlayerpoint = this._map.latLngToLayerPoint(hotspotlatlng),
							hotspotBounds = EzServerClient.bounds(new Point(hotspotlayerpoint.x - 6, hotspotlayerpoint.y - 6), new Point(hotspotlayerpoint.x + 6, hotspotlayerpoint.y + 6));
						var data = {
							'ID': hotspotArr[i].ID,
	...
	EzServerClient.Marker = EzServerClient.Class.extend({
		...
		updateTransformRotate : function (){
			this._icon.style.transform +='rotate('+this.options.angel+'deg)';
		},
		update: function () {
			if (this._icon) {
		...
		_setPos: function (pos) {
			...
			//modify by zhouyx
			//_zIndex取消使用pos节点高度pos.y
			this._zIndex =  this.options.zIndexOffset;
			this._angel = this.options.angel;
			this._resetZIndex();
		},
		_updateZIndex: function (offset) {
			this._icon.style.zIndex = this._zIndex + offset;
			this.updateTransformRotate();
		},
	...
	*/
	...
	EzServerClient.FeatureObject = EzServerClient.Class.extend({
		initialize: function(uLayer) {
			...
			if (uLayer == null) {
				throw new Error("���ڹ��캯���д���ͼ������!")
			}
			this.layInfo = uLayer;
	
			// 20091225����������
			/**
			 * @description <font color=orange size=2>���Ժ��壺Ҫ�ص��ֶζ��󼯺�<br>
			 *              ��ʼĬ��ֵ��[]</font><br>
			 * @return {Array}
			 * @field
			 * @private
			...
			this.ezFields = [];
			/**
			 * @description <font color=orange size=2>���Ժ��壺Ҫ�ص����ĵ�<br>
			 *              ��ʼĬ��ֵ��null</font><br>
			 * @return {Point}
			 * @field
			 * @private
			 ...
			 *              size=2>���Ժ�����ʾ�ֶ�����Ӧ��ֵ��this.dispname�����������ʲ����⣬��V6.3.5�����Ժ��汾��dispFieldValue��������<br>
			 *              ��ʼĬ��ֵ����</font><br>
			 * @return {String}
			 * @field
			 * @private 20091103
		...
		getMBR:function(){
			// var pMBR = EzServerClient.GlobeFunction.getPathMBR(this.linestr);// ��ȫ�ֺ�����initalize.js�ж���
			// return pMBR;
		},
		...
		setCenterPoint:function(vPoint){
			try {
				if (!(EzServerClient.GlobeFunction.isTypeRight(vPoint, EzServerClient.Point)))
					throw EzErrorFactory.createError("FeatureObject::setCenterPoint������arguments[0]���Ͳ���ȷ");
				else {
					this.centerPoint = vPoint;
					this.point = this.centerPoint;
				}
			} catch (e) {
				throw EzErrorFactory.createError("FeatureObject::setCenterPoint�����в���ȷ",e);
			}
		},
			...
			try {
				if (!EzServerClient.GlobeFunction.isTypeRight(vEzShapeArr, "Array")) {
					throw EzErrorFactory
							.createError("EzLayer::setSpatialFilterShapes������arguments[0]�������Ͳ���ȷ");
				}
				this.spatialFilterShapes = vEzShapeArr;
			} catch (e) {
				throw EzErrorFactory.createError(
						"EzLayer::setSpatialFilterShapes����ִ�в���ȷ", e);
			}
		},
		...
		setSpatialColumn:function(vSpatialColumn){
			try {
				if (!EzServerClient.GlobeFunction.isTypeRight(vSpatialColumn, "string")) {
					throw EzErrorFactory.createError("EzLayer::setSpatialColumn������arguments[0]�������Ͳ���ȷ");
				}
				this.spatialColumn = vSpatialColumn;
			} catch (e) {
				throw EzErrorFactory.createError("EzLayer::setSpatialColumn����ִ�в���ȷ", e);
			}
		},
		...
		setGeometryType:function(vGeometryType){
			try {
				if (!(EzServerClient.GlobeFunction.isTypeRight(vGeometryType, "string"))) {
					throw EzErrorFactory
							.createError("EzLayer::setGeometryType������arguments[0]���Ͳ���ȷ");
				} else {
					this.geometryType = vGeometryType;
				}
			} catch (e) {
				throw EzErrorFactory.createError("EzLayer::setGeometryType�����в���ȷ", e);
			}
		},
		...
		setFeatures:function(vFeatures){
			try {
				if (!(EzServerClient.GlobeFunction.isTypeRight(vFeatures, "Array"))) {
					throw EzErrorFactory
							.createError("EzLayer::setFeatures������arguments[0]���Ͳ���ȷ");
				} else {
					this.features = vFeatures;
				}
			} catch (e) {
				throw EzErrorFactory.createError("EzLayer::setFeatures�����в���ȷ", e);
			}
		}
	});
	...
	EzServerClient.EzShape = EzServerClient.Class.extend({
		...
		setGeometryType:function(vGeometryType){
			try{
				if( !(EzServerClient.GlobeFunction.isTypeRight(vGeometryType, "string"))){
					throw EzErrorFactory.createError("EzShape::setGeometryType������arguments[0]���Ͳ���ȷ");
				}else{
					this.geometryType = vGeometryType;
				}
			}catch(e){
				throw EzErrorFactory.createError("EzShape::setGeometryType�����в���ȷ", e);
			}		
		},
		...
		setCoordinateSequence:function(vCoordinateSequence){
			try{
				if( !(EzServerClient.GlobeFunction.isTypeRight(vGeometryType, "string"))){
					throw EzErrorFactory.createError("EzShape::setCoordinateSequence������arguments[0]���Ͳ���ȷ");
				}else{
					this.coordinateSequence = vCoordinateSequence;
				}
			}catch(e){
				throw EzErrorFactory.createError("EzShape::setCoordinateSequence�����в���ȷ", e);
			}
		}
	});
	...
	Polyline = EzServerClient.Class.extend({
		initialize: function(points, color, weight, opacity, isShow) {
		...
		//yxp
		setOpacity: function (opacity) {
			this.uPolyline.options.opacity = opacity;
			if (this.uPolyline._container) {
				this.uPolyline._updateStyle();
			}
		},
		getCenterPoint: function () {
			var points = this.uPolyline.getLatLngs();
			var sx = points[0].lng; var sy = points[0].lat;
			var ex = points[points.length - 1].lng; var ey = points[points.length - 1].lat;
		...
		getOpacity: function () {
			return this.uPolyline.options.opacity;
		},
		addTo: function(map) {
			this.map_ = map;
		...
		generateContent: function(options) {
			...
			var content = "<marker id='arrowHead"+options.id+"' viewBox='" + viewBox + "' markerWith='" + width +
				"' markerHeight='" + height + "' refx='1' refy='5' orient='auto'><path d='" + triangle + "' fill='" + fill + "'/></marker>";
		...
		addArrowHead: function(options) {
			options.id=this.randomString(32);
			...
			path.setAttribute('marker-end', 'url(#arrowHead'+options.id+')');
			this.uPolyline.options.arrowhead = true;
		},
		getArrowhead: function () {
		   return this.uPolyline.options.arrowhead;
		},
	...
	Polygon = EzServerClient.Class.extend({
		...
		getWeight: function () {
			return this.uPolygon.options.weight;
		},
		getGeometryType: function() {
			return "Polygon";
		},
	...
	/**
	* modify by zhouyx 2017/6/8
	* EzServerClient 初始化方法
	* @param id    container
	* @param isMap3D    是否为3维地图默认false
	* @returns {*|void}
	*/
	EzMap = function(container,isMap3D){
		//container=reloadMapContainer(container, isMap3D);
		if(isMap3D){
			return new PMap("cesiumContainer");
		}else{
			return new EzMapOld(container);
		}
	}
		 ...
		 * @param isMap3D
		 */
		this.reloadMapContainer = function (container, isMap3D) {
			if(!this.webgl){
				var parent=container.parentNode;
				this.webgl = document.createElement("DIV");
				this.webgl.id = "cesiumContainer";
				parent.appendChild(this.webgl);
			}
			if (!isMap3D) {
				this.webgl.style.width=0;
				this.webgl.style.height=0;
				...
				this.webgl.style.width="1000px";
				this.webgl.style.height="750px";
				EzServerClient.is3DMap = isMap3D;
			}
			return this.webgl;
		}
	...
	/**
	* modify by zhouyx on 2017/6/8
	* pMap对象
	* @type {{}}
	*/
	
	var PMap=EzServerClient.Class.extend({
		/**
		 * 初始化webgl场景
		 * @param  contair 容器
		 * @returns {*|void}
		 */
		initialize: function (contair) {
		   /*
		   open方式打开图层
		   this.viewer = new Cesium.Viewer('mapDiv');
			this.viewer.scene.open(ezMap.Layer3D.realspaceUrl);
			var that=this;
			setTimeout(function () {
				that.viewer.flyTo(that.viewer.imageryLayers.get(2));
			},3000);*/
			//this.viewer.flyTo(this.viewer.imageryLayers._layers[6]);
			var that=this;
			this.viewer = new Cesium.Viewer(contair,{
				navigation : true
			});
			 this.scene = this.viewer.scene;	
			if(ezMap.Layer3D.scene){
				if(ezMap.Layer3D.scene.minimumZoomDistance){
					this.scene.screenSpaceCameraController.minimumZoomDistance=ezMap.Layer3D.scene.minimumZoomDistance;
				}
				if(ezMap.Layer3D.scene.maximumZoomDistance){
					this.maxDistanceHandler = new Cesium.ScreenSpaceEventHandler(this.scene.canvas);
					this.maxDistanceHandler.setInputAction(function(e) {
						that.distance=that.cameraHeight()-ezMap.Layer3D.scene.maximumZoomDistance;
						if(that.distance>0){
							that.viewer.camera.moveForward(that.distance);
						}
					}, Cesium.ScreenSpaceEventType.WHEEL);
				}
				if(ezMap.Layer3D.scene.showDistance){
					this.showDistanceHandler = new Cesium.ScreenSpaceEventHandler(this.scene.canvas);
					this.showDistanceHandler.setInputAction(function(e) {
		...
		initialize2: function () {
			try{
				if (ezMap.Layer3D) {
					this.terrainProvider(ezMap.Layer3D.terrainProvider);
					this.S3MTilesLayer(ezMap.Layer3D.S3MTilesLayer);
					this.imageProvider(ezMap.Layer3D.imageProvider);
				}
				this.entityClick();
			}catch(e){
				throw new Error(e);
		 ...
		 * @options 地形所需参数
		 */
		terrainProvider: function (options) {
			this.viewer.terrainProvider = new Cesium.CesiumTerrainProvider(options);
		},
		/**
		 * 添加影像图层
		 *@array 格式array[options]
		 * 多个影像对象参数顺序执行
		 */
		imageProvider: function (options) {
		   if (options && options.length > 0) {
				var layer,imageryLayers = this.viewer.imageryLayers;
				for (var i = 0; i < options.length; i++) {
					if(options[i].isWMTS){
						layer=new Cesium.WebMapTileServiceImageryProvider({
							url: options[i].url,
							format: options[i].format,
							tileMatrixSetID:options[i].tileMatrixSetID,//Custom_China,GeogTDT,Matrix_0,GCS_China_Geodetic_Coordinate_System_2000,GCS_China_Geodetic_Coordinate_System_2000
							layer: options[i].name,
							style : 'default',
							rectangle:Cesium.Rectangle(options[i].rectangle[0],options[i].rectangle[1],options[i].rectangle[2],options[i].rectangle[3])
						});
						layer._tilingScheme=new Cesium.GeographicTilingScheme({ ellipsoid : Cesium.Ellipsoid.WGS84 });
						layer.__proto__.requestImage = function(x, y, level) {
							level+=1;
							var url=this.url;
							url=url.replace("{x}",x);
							url=url.replace("{y}",y);
						...
						layer=imageryLayers.addImageryProvider(layer);
					}else{
						layer=imageryLayers.addImageryProvider(new Cesium.SuperMapImageryProvider(options[i]));
						if(options[i].isFly){
							var yy=this.viewer.flyTo(layer);
							var that=this;
							yy.then(function (e) {
								that.flyToEnd=e;
							});
		 ...
		 * 多个对象参数顺序执行
		 */
		S3MTilesLayer: function (options) {
			var xx,layers={};
			if (options && options.length > 0) {
				for (var i = 0; i < options.length; i++) {
					xx=this.viewer.scene.addS3MTilesLayerByScp(options[i].url,{
						name : options[i].name
					});
					layers[options[i].name]=options[i];
					if(options[i].max&&options[i].min){
						xx.then(function(layer){
							layer.VisibleDistanceMax=layers[layer._name].max;
							layer.VisibleDistanceMin=layers[layer._name].min;
						});
					}
				}
			}
		 ...
		 * @param options
		 */
		xyto3dPoint:function(options){
			var that=this;
			this.gridCellQueryParam = new SuperMap.REST.GetGridCellInfosParameter({
				datasetName: ezMap.Layer3D.gridRest.datasetName,
				dataSourceName: ezMap.Layer3D.gridRest.dataSourceName,
				X: parseFloat(options.lon),
				Y: parseFloat(options.lat)
			});
			if(!this.querySuccess){
				this.querySuccess=function(evt){
					if(options.callback){
						options.callback({
							x:evt.object.X,
							y:evt.object.Y,
							z:evt.result&&(evt.result.value+10)
						});
					}
				}
			...
			if(!this.queryFailed){
				this.queryFailed=function(evt) {
					throw new Error(evt);
				}
			}
			if(!this.gridCellQueryService){
				this.gridCellQueryService = new SuperMap.REST.GetGridCellInfosService(ezMap.Layer3D.gridRest.url, {
					'eventListeners': {
						'processCompleted': that.querySuccess,
						'processFailed': that.queryFailed
					}
				});
			}
	
			this.gridCellQueryService.processAsync(this.gridCellQueryParam);
		...
		entityClick:function(){
			...
			this.handler.setInputAction(function(e){
				that.viewer.selectEntity=that.viewer._selectedEntity;
			},Cesium.ScreenSpaceEventType.LEFT_CLICK);
			that.billboardDelDom();
		},
		/**
		 * 设置相机视角
		 * @param options
		 */
		setCameraView:function(options){
			var camera=this.viewer.camera;
			//camera.position
			//Cesium.Cartesian3.fromDegrees(options.x, options.y,options.z)
			this.viewer.camera.setView({
				destination: camera.position,
				orientation: {
					heading : camera.heading+0.00001214,  //左右偏移
					pitch : camera.pitch +0.00001214,   //上下偏移
					roll : camera.roll+0.00001214
				}
			});
		},
		/**
		 ...
		 */
		bbox:function(){
			var localRectangle = this.viewer.camera.computeViewRectangle();
			return localRectangle.south * 180.0 / Math.PI + ","
		...
		billboard:function(options){
			...
			if(!options.position){
				options.position=Cesium.Cartesian3.fromDegrees(options.lon,options.lat);
			}
	
			var id=options.id||Math.uuid(32);
			var heightReference=Cesium.HeightReference.CLAMP_TO_GROUND;
	
			if(!options.labelBackgroundColor){
				options.labelBackgroundColor="#c9f6d4";
			}
			...
			var label={
				...
				fillColor : Cesium.Color.fromCssColorString(options.fontColor),
				showBackground:true,
				backgroundColor: Cesium.Color.fromCssColorString(options.labelBackgroundColor),
				font:options.fontSize+"px Helvetica",
				pixelOffset : new Cesium.Cartesian2(0, -6)
			};
			
			if(options&&options.isNotLabel){
				label={};
			}
			
			if(options.markerNum){
				label={
					text:options.markerNum+"",
					verticalOrigin : Cesium.VerticalOrigin.BOTTOM,
					heightReference:Cesium.HeightReference.CLAMP_TO_GROUND,
					fillColor : Cesium.Color.fromCssColorString("#ffffff"),
					showBackground:true,
					backgroundColor: Cesium.Color.fromCssColorString("#ff9222"),
					font:"18px Helvetica",
			...
			billboard.setAngel=this.setAngel;
			billboard.setZIndex=this.setZIndex;
			billboard._lon=options.lon;
			billboard._lat=options.lat;
			billboard._des=options.description;
			options.item=billboard;
			if(options.isSelect){
				this.viewer.selectEntity=billboard;
				this.viewer.selectedEntity = billboard;
			}
			return billboard;
		},
		/**
		 ...
		 * @param options
		 */
		billboardDelDom:function(options){
			var that = this;
			$("#entities").remove();
			$(".cesium-infoBox-camera").after('<button type="button" id="entities" ' +
				'class="cesium-infoBox-close cesium-infoBox-del" title="删除" >D</button>');
			// allow-modals
			this.viewer.infoBox.frame.sandbox = "allow-same-origin allow-scripts allow-popups allow-forms";
			$("#entities").on("click", function () {
				$(this).next().click();
				that.viewer.entities.removeById(that.viewer.selectEntity._id + "_arrow");
				that.viewer.entities.removeById(that.viewer.selectEntity._id + "_polyline");
	
				that.viewer.entities.remove(that.viewer.selectEntity);
				if (that.delEntityEvent) {
					that.delEntityEvent();
				}
			});
		},
		mulBillboard:function(options){
			var that=this;
			that.billboards = that.viewer.scene.primitives.add(new Cesium.BillboardCollection());
			var len=options.groups.pois&&options.groups.pois.length,item;
			for(var i=0;i<len;i++){
				item=options.groups.pois[i];
				options.item=item;
				that.billboards.add({
					position : new Cesium.Cartesian3.fromDegrees(item.x, item.y),
					description:that.description(that.itemOptions(options)),
		...
		addPolygon:function(options){
			var array=options.path.split(","),
				positions=new Array();
			if(!array){
				throw new Error("路线空间数据不能为空！");
			}
			for(var i=0;i<array.length;i++){
				positions.push(parseFloat(array[i]));
			}
			var id=options.id||Math.uuid(32);
			var color,opcity,outlineColor;
			opcity=options.opacity||0.3;
			...
			var polygon=this.viewer.entities.add({
				...
				polygon : {
					...
					material : color.withAlpha(opcity),
					heightReference:Cesium.HeightReference.CLAMP_TO_GROUND
				  /*  outline:true,
			...
			if(options.isSelect){
				this.viewer.selectEntity=polygon;
		...
		addPolyline:function(options){
			...
			for(var i=0;i<array.length;i++){
				if(i==array.length-4){
					startX=parseFloat(array[i]);
				}else if(i==array.length-3){
					startY=parseFloat(array[i]);
				}else if(i==array.length-2){
					endX=parseFloat(array[i]);
				}else if(i==array.length-1){
					endY=parseFloat(array[i]);
				}
				positions.push(parseFloat(array[i]));
			}
		   var color,arrowPosition;
		   if(options.color){
	
	
			   color=Cesium.Color.fromCssColorString(options.color).withAlpha(options.opacity||1);
		   }else{
			   color=Cesium.Color.RED;
		   }
		   this.height=this.viewer.scene.globe.ellipsoid.cartesianToCartographic(this.viewer.camera.position).height;
		   if(this.height<7000){
			   options.width=options.width/3;
		   }
	
			var id=options.id||Math.uuid(32);
				if(options&&options.isArrow){
					var array=this.calculateArrow({
						startX:startX,
						startY:startY,
						endX:endX,
						endY:endY,
				...
				if(options.isSelect){
					this.viewer.selectEntity=line;
					this.viewer.selectedEntity = line;
				}
		...
		calculateArrow:function(options){
	
			var sx=options.startX,sy=options.startY,ex=options.endX,ey=options.endY;
			var screenS =Cesium.SceneTransforms.wgs84ToWindowCoordinates (this.viewer.scene, Cesium.Cartesian3.fromDegrees(sx, sy)),
				screenE =Cesium.SceneTransforms.wgs84ToWindowCoordinates (this.viewer.scene, Cesium.Cartesian3.fromDegrees(ex, ey));
	
			if(!screenS||!screenE){
				return;
			}
	
			sx=screenS.x,sy=screenS.y,ex=screenE.x,ey=screenE.y;
	
			var L=options.width||15;
			var H=L/3;
	
	
			// 计算坐标
			function rotateVec(px,py,ang,isChLen,newLen) {
				var mathstr = new Array();
			...
			var arraow_len = Math.sqrt(L * L + H * H)+5; // 箭头的长度
			var arrXY_1 = rotateVec(ex - sx, ey - sy, awrad, true, arraow_len);
			var arrXY_2 = rotateVec(ex - sx, ey - sy, -awrad, true, arraow_len);
			var arrXY_3 = rotateVec(ex - sx, ey - sy, awrad / 3, true, arraow_len / 2);//上
			var arrXY_4 = rotateVec(ex - sx, ey - sy, -awrad / 3, true, arraow_len / 2);//下
			var arrXY_5 = rotateVec(ex - sx, ey - sy, 0, true, arraow_len/2 );//中
			if(!arrXY_1||
				!arrXY_2||
				!arrXY_3||
				!arrXY_4){
			...
			var x_3 = ex - arrXY_1[0]; // (x3,y3)是第一端点
			var y_3 = ey - arrXY_1[1];
			var x_4 = ex - arrXY_2[0]; // (x4,y4)是第二端点
			var y_4 = ey - arrXY_2[1];
			var x_5 = ex - arrXY_3[0]; // (x5,y5)是 上
			var y_5 = ey - arrXY_3[1];
			var x_6 = ex - arrXY_4[0]; // (x6,y6)是 下
			var y_6 = ey - arrXY_4[1];
			var x_7 = ex - arrXY_5[0]; // (x7,y7)是 中
			var y_7 = ey - arrXY_5[1];
	
	
			var x5= this.viewer.camera.pickEllipsoid(new Cesium.Cartesian2(x_5,y_5), ellipsoid),
				x6= this.viewer.camera.pickEllipsoid(new Cesium.Cartesian2(x_6,y_6), ellipsoid);
	
			var ellipsoid=this.viewer.scene.globe.ellipsoid;
			var cartesian = ellipsoid.cartesianToCartographic(x5);
			var cartesian1 = ellipsoid.cartesianToCartographic(x6);
	
			result.push(Cesium.Math.toDegrees(cartesian.longitude));
			result.push(Cesium.Math.toDegrees(cartesian.latitude));
	
			result.push(options.endX);
			result.push(options.endY);
	
	
			result.push(Cesium.Math.toDegrees(cartesian1.longitude));
			result.push(Cesium.Math.toDegrees(cartesian1.latitude));
		...
		/**
		 * coefficient缩放系数越小缩放度越大
		 * @param coefficient
		 */
		zoomIn: function(coefficient) {
			if(this.cameraHeight()<3400){
			   return;
			}
			if(!coefficient){
				coefficient=200.0;
			}
			this.viewer.camera.zoomIn(this._cameraPosition().height/coefficient);
	
		},
		zoomOut: function(coefficient) {
			if(!coefficient){
				coefficient=200.0;
			}
			this.viewer.camera.moveBackward(this._cameraPosition().height/coefficient);
		},
		/**
		 * 相机位置
		 * wps84
		 * @private
		 */
		_cameraPosition:function(){
			return this.viewer.scene.globe.ellipsoid.
			cartesianToCartographic(this.
		 ...
		 * @param roll
		 */
		gotoCenter: function(x,y,z,heading,pitch,roll) {
			this.scene.camera.setView({
				destination : Cesium.Cartesian3.fromDegrees(x,y,z||0),
				orientation : {
					heading : heading||0.0010316700982784255,//方位角
					pitch : pitch||-1.5707963267948966,//俯仰角
					roll : roll||0//滚动角
				}
			});
		},
		 ...
		 * @param callback
		 */
		addMapEventListener: function(eventType, handler,callback) {
			if(eventType=="LEFT_CLICK"){
				try{
					...
					throw new Error("三维地图不存在事件");
				}
			}
		},
	
		addOverlay: function(overlay,options) {
			if("polygon"==options.type){
				return this.addPolygon(options);
			}
		},
	
		changeDragMode: function (drawMode, callback, state, obj, drawclor, options) {
			var that=this;
			...
			switch (drawMode) {
				...
				case "measure":
					...
					break;
				case "drawPolyline":
					this._drawPolyline(callback, drawclor,options);
					break;
				case "drawPolyline2":
					this._drawPolyline(callback, drawclor,options);
					break;
				case "drawRect":
					...
					break;
				case "drawPolygon":
					this._drawPolygon(callback,options);
					break;
				case "drawCircle":
		...
		_drawPoint: function(callback) {
			...
			if(!this.drawPointHandler){
				this.drawPointHandler = new Cesium.ScreenSpaceEventHandler(this.viewer.scene.canvas);
			}
			var viewer=this.viewer,
				scene=this.viewer.scene,
				camera=this.scene.camera,
				that=this;
			this.drawPointHandler.setInputAction(function (e) {
				...
				var hx=Math.ceil(viewer.camera.positionCartographic.height);
	
				var description=that.description({
					values:[longitude, latitude, height,
						camera.heading,
						camera.pitch,
						camera.roll,
						hx]
				...
				callback&&callback({
					...
					lat:latitude,
					height:height,
					position:Cesium.Cartesian3.fromDegrees(longitude, latitude , height + 30),
					description:description
				});
				that.drawPointHandler.destroy();
				that.drawPointHandler=null;
			},Cesium.ScreenSpaceEventType.LEFT_CLICK);
		},
		updateBillboard:function(billboard,options){
			this.viewer.entities.remove(billboard);
			return this.billboard(options);
		},
		_measureTool2: function(callback) {
	
			if(this.mesureLine){
				this.viewer.entities.remove(this.mesureLine);
				this.distanceLabel=null;
			}
	
			this.viewer.entities.removeById("distanceLabel");
			var that=this;
			this._drawPolyline(function(options){
				var position=options.positions[options.positions.length-1];
				that.mesureLine=that.addPolyline({
					path:options.coordString,
					width:15,
					name:"测线",
					color:"red"
				});
				...
				if(options.distance >= 1000){
					...
					that.distanceLabel=that.viewer.entities.add({
						id:"distanceLabel",
						position : position,
						label : {
							text : options.distance.toFixed(2) + '千米',
							verticalOrigin : Cesium.VerticalOrigin.BOTTOM,
							heightReference:Cesium.HeightReference.CLAMP_TO_GROUND,
							font:"24px Helvetica",
							pixelOffset : new Cesium.Cartesian2(0, -6),
					...
					that.distanceLabel=that.viewer.entities.add({
						...
						position : position,
						label : {
							text : options.distance.toFixed(2) + '米',
							verticalOrigin : Cesium.VerticalOrigin.BOTTOM,
							heightReference:Cesium.HeightReference.CLAMP_TO_GROUND,
							font:"24px Helvetica",
							pixelOffset : new Cesium.Cartesian2(0, -6),
		...
		_drawPolyline: function (callback, drawColor,options) {
			...
			var that=this,distance=0;
			this.polylineHandler._clampMode = 1;
			this.polylineHandler.handler.mode=3;
			this.polylineHandler.activate();
	
			this.polylineHandler.drawEvt.addEventListener(function(result){
				that.polylineHandler.clear();
				var positions=result.object.positions,cartographic,longitude,latitude,
					coordString='',distance=0;
				for(var i = 0, len = positions.length - 1; i < len ; i++){
					cartographic = Cesium.Cartographic.fromCartesian(positions[i]);
					...
					coordString+=longitude+","+latitude+",";
				}
		...
		_drawPolygon: function(callback,options) {
			...
			if(!this.polygonHandler){
				this.polygonHandler = new Cesium.DrawHandler(this.viewer,Cesium.DrawMode.Polygon);
			}
			...
			var that=this,distance=0;
			this.polygonHandler._clampMode = 1;
			this.polygonHandler.handler.mode=3;
			this.polygonHandler.activate();
			this.polygonHandler.drawEvt.addEventListener(function(polygon){
				  if(!options||!options.isNotClear){
					  that.polygonHandler.clear();
				  }
				var positions = polygon.object.positions;
				var array = [],cartographic,longitude,latitude,coordString='',height;
				...
				that.polygonHandler.deactivate();
			});
		...
		clear: function() {
			...
			if(Cesium.defined(scene.currentPolyline)){
				this.polylineHandler.polylines.remove(scene.currentPolyline);
		...
		measureArea: function(data) {
			var viewer=this.viewer;
			var that=this;
			that.viewer.entities.removeById("measureArea");
			that.viewer.entities.removeById("measureArea_polyline");
			that.viewer.entities.removeById("areaLabel");
	
			this._drawPolygon(function (ploygon) {
				var array=ploygon.array;
				var path=ploygon.coordString;
				that.areaLine=that.addPolygon({
					id:"measureArea",
					path:path,
					width:15,
					name:"测面",
				...
				var arrs = new Array();
				var tems = new Array();
				var lastPoint=ploygon.lastPoint;
				arrs.push(tems);
				...
				if(area >= 1000000){
					area /= 1000000;
					that.areaLabel=viewer.entities.add({
						id:"areaLabel",
						position : Cesium.Cartesian3.fromDegrees(lastPoint.x, lastPoint.y, lastPoint.z),
						label : {
		...
		_showMessageInfo:function (point) {
			if(!this.popup){
				var container=document.getElementByClassName("cesium-viewer-cesiumWidgetContainer");
				this.popup = EzServerClient.DomUtil.create('div', 'leaflet-draw-tooltip', container);
				EzServerClient.DomUtil.addClass(this.popup, 'leaflet-draw-tooltip-single');
			}
			this.popup.style.display = '';
			this.popup.innerHTML ='<span>右击完成标绘画</span>';
			this.popup.style.visibility = 'inherit';
	
			EzServerClient.DomUtil.setPosition(this.popup, point.screen);
		},
		_messageInfo:function(){
			if(!this.handler){
			...
			handler.setInputAction(function(movement) {
				...
				var clickPt =Cesium.SceneTransforms.wgs84ToWindowCoordinates (viewer.scene, movement.endPosition);
				if (cartesian) {
					//将笛卡尔坐标转换为地理坐标
					...
					that._showMessageInfo({
						x:longitudeString,
						y:latitudeString,
						screen:{screenX:clickPt.x,
							screenY:clickPt.y}
					});
				}else {
		...
		measureLength: function(data) {
			var viewer=this.viewer;
			var that=this;
			this._messageInfo();//添加提示信息事件
			this._drawPolyline(function (polyline) {
				var path=polyline.coordString,
		...
		openInfoWindow: function(point, strHTML, bIsInScreen,options) {
		...
		description:function(options){
			...
			if(!options.titles){
				options.titles = ['经度','纬度','高度','相机方位角','相机俯仰角','相机旋转角度','相机高度'];
			}
			if(!options.values){
				options.values = ["默认","默认","默认","默认","默认","默认","默认"];
			}
	
			var html = '';
	
			for ( var i=0;i<options.titles.length;i++) {
				html += '<tr><td>' + options.titles[i] + '</td><td>' + options.values[i] + '</td></tr>';
			}
			if (html.length > 0) {
		...
		removeOverlay: function(entity,isArrow) {
			if(entity){
				this.viewer.entities.removeById(entity._id+"_arrow");
				this.viewer.entities.removeById(entity._id+"_polyline");
	...
	/**
	* modify by zhouyx on 2017/6/8
	* EzMap 原ezmap 构造函数 更名为EzMapOld
	* @type {{}}
	*/
	var EzMapOld = EzServerClient.Class.extend({
		statics: {
		...
		initialize: function(id) {
			...
			this.uMap = new EzServerClient.Map(id, {
				center: EzMapOld.pointCoorType(ezMap.CenterPoint[0], ezMap.CenterPoint[1]),
				crs: crsobj,
		...
		addOverView: function(ov) {
			...
			if(ov.miniMap._toggleDisplayButton){
		...
		changeDragMode: function(drawMode, callback, state, obj,drawclor,options) {
			...
			switch (drawMode) {
				...
				case "measure":
					...
					break;
				case "drawPolyline":
					this._drawPolyline(callback, drawclor);
					break;
				case "drawRect":
					this._drawRect(callback);
					break;
				case "drawPolygon":
					...
					break;
				case "drawPolygon2":
					this._drawPolygon2(callback,state);
					break;
				case "drawCircle":
					this._drawCircle(callback);
					break;
		...
		_drawPoint: function(callback) {
			this.uMap.getContainer().style.cursor = 'pointer';
			var output = callback;
	
			var that = this;
			this.modeStatus = "drawPoint";
			this.uMap._drawPoint=_drawPoint;
			this.uMap.on("click", _drawPoint);
			this.uMap.on("destruct::drawPoint", _destruct);
			this.uMap.on("mousemove", _drawPointMove);
			this.popup = EzServerClient.DomUtil.create('div', 'leaflet-draw-tooltip', this.uMap._panes.popupPane);
			EzServerClient.DomUtil.addClass(this.popup, 'leaflet-draw-tooltip-single');
			this.popup.style.display = '';
			this.popup.innerHTML ='<span>单击设置标注点</span>';
	
	
			function MessgeInfo(latlng){
				that.popup.style.visibility = 'inherit';
				EzServerClient.DomUtil.setPosition(that.popup, that.uMap.latLngToLayerPoint(latlng));
			}
	
	
			function _drawPointMove(e) {
				MessgeInfo(e.latlng);
			}
			...
			function _drawPoint(e) {
				if(that.popup){
					that.uMap._panes.popupPane.removeChild(that.popup);
				}
		...
		_measureTool2: function(callback) {
			var that=this;
			this.uMap.getContainer().style.cursor = 'pointer';
			this.uMap.measureLength = new EzServerClient.Polyline.Measure(this.uMap, {});
			this.uMap.measureLength.setCallBackFunction(callback);
			this.uMap.measureLength.enable.call(this.uMap.measureLength);
			this.uMap.disableMeasureLength=function(){
				that.uMap.measureLength.disable.call(that.uMap.measureLength);
			}
		},
	
		_drawPolyline2:function(callback){
			var that=this;
			this.uMap.measureArea = new EzServerClient.Polyline.Measure(this.uMap, {isMeasure:false});
			this.uMap.measureArea.setCallBackFunction(callback);
			this.uMap.measureArea.enable.call(this.uMap.measureArea);
			this.uMap.disableMeasureArea=function(){
				that.uMap.measureArea.disable.call(that.uMap.measureArea);
			}
		},
		...
		_drawPolyline: function (callback, drawclor) {
			this.uMap.getContainer().style.cursor = 'pointer';
			var _drawing = true;
			var output = callback;
			var _poly = new EzServerClient.Polyline([], {
			...
			EzServerClient.DomUtil.addClass(this.popup, 'leaflet-draw-tooltip-single');
			this.popup.style.display = '';
			this.popup.innerHTML ='<span>单击设置起点</span>';
			...
			function MessgeInfo(latlng){
				that.popup.style.visibility = 'inherit';
				EzServerClient.DomUtil.setPosition(that.popup, that.uMap.latLngToLayerPoint(latlng));
				if(_poly._latlngs.length>0){
					that.popup.innerHTML ='<span>右击完成绘制</span>';
				}
			}
		...
		_drawPolygon2:function(callback){
			var measureTool2Handler = new EzServerClient.Draw.Polygon(this.uMap, {isMeasure:false});
			//添加结束回掉事件
		...
		_drawPolygon: function(callback,options) {
			...
			var isNotLabel=options&&options.isNotLabel;
			this.popup = EzServerClient.DomUtil.create('div', 'leaflet-draw-tooltip', this.uMap._panes.popupPane);
			if(!isNotLabel){
				EzServerClient.DomUtil.addClass(this.popup, 'leaflet-draw-tooltip-single');
				this.popup.style.display = '';
	...
	EzServerClient.Marker.include({
		bindTitle:function(tit){
			...
			if (this.title) {
				...
				if (typeof tit === 'string') {
					//this.title.setContent(tit);
					this.title = new Title(tit, 12, 7, "宋体", "red", "#e4f2fc", "#00B2FF", "1");
				} else {
					this.title = tit;
				}
			}
	...
	/**
	 * [QueryItem description] ���ǲ�ѯ��
	 * @type {[type]}
	 */
	EzServerClient.EasyXmlParse.QueryItem = EzServerClient.EasyXmlParse.extend({
		options: {
			timeOut: null, //��ѯ��ʱʱ�� ��ʼĬ��ֵ����
			tryTime: null, //��ѯ��ʱʱ�� ��ʼĬ��ֵ����
			state: State.SUCCESS, //���õ�Ĭ�ϵ��Ӳ�ѯ״̬ ��ʼĬ��ֵ��true
			failureOnError: false //��ѯ�����ǲ���һ������ ��ʼĬ��ֵ��false
		},
		...
		initialize: function (url, xmlDoc, options) {
			...
			this.currentTryTime = 0; //��ǰ��ѯ�������� ��ʼĬ��ֵ��0
			this.reSendId = 0; //����ID ��ʼĬ��ֵ��0
			this.responseXml = null; //���ص�Xml ��ʼĬ��ֵ����
		},
		...
		doQuery: function () {
			...
			/* �����Ӳ�ѯ���ԣ��������Ե�ID��pMe.reSendId���Ӳ�ѯ����ԭ�е�������Ԫ�� */
			else
				RequestMapObject.add(xmlhttp, new RequestMapItemBinding(pMe.reSendId,
					text2Send));
			xmlhttp.send(text2Send);
	
			/* ֻ��û�����ԣ�EzServerClient.EasyXmlParse.EzServerClient.EasyXmlParse.EzMapserviceQuery.NextOrder++������ */
			if (pMe.currentTryTime == 0)
				EzServerClient.EasyXmlParse.EzMapserviceQuery.NextOrder++;
			...
			function HandleStateChange() {
				if (xmlhttp.readyState == 4) {
					...
					if (xmlhttp.status == 200) {
						...
						//doc1.async = false;
						//doc1.loadXML(xmlhttp.responseText);  //���ط��ص�xml
						if ((doc1.getElementsByTagName("ERROR").length == 0)) { //���ص�xmlû�д�����Ϣ
							var o = RequestMapObject.getValue(xmlhttp);
							RequestMapObject.remove(xmlhttp);
							pMe.setResponseXml(xmlhttp.responseText); //���÷�����Ϣ
							pMe.setQueryXml(RequestMapObject.getValue(xmlhttp).text); //��ȡ���͵�xml������
							pMe.itemId = RequestMapObject.getValue(xmlhttp).num; //��ȡ���͵�xml������
							if (--pMe.mapServiceQuery.options.itemCurrentCount == 0) { //�����Ӳ�ѯ�����ɵ�����
								EzServerClient.EasyXmlParse.EzMapserviceQuery.queryState = false; //������ʱ������ѯ�����Ѿ�����
								pMe.mapServiceQuery.onFinished();
							}
						} else { //���ص�xml����������Ϣ
							if (++pMe.currentTryTime > pMe.getTryTime()) { //�жϴ�ʱ��ǰ�ز������Ƿ񳬹��趨���ز�����
								pMe.setResultState(State.ERROR); //�����Ӳ�ѯ����״̬
								pMe.mapServiceQuery.options.queryFlag++;
								--pMe.mapServiceQuery.options.itemCurrentCount;
								if (pMe.mapServiceQuery.options.failureOnError //�ò�ѯ������һ�����������Ӳ�ѯ�г�����ֱ���״�
									&& pMe.mapServiceQuery.options.queryFlag == 1) {
									EzServerClient.EasyXmlParse.EzMapserviceQuery.queryState = false;
									throw new Error("���ڵ�" + RequestMapObject.getValue(xmlhttp).num + "���Ӳ�ѯ�������ò�ѯ������ֹ");
								}
								pMe.setResponseXml(xmlhttp.responseText); //���÷�����Ϣ
								if (pMe.mapServiceQuery.options.itemCurrentCount == 0) { //�Ӳ�ѯ�Ѿ������ˣ����ڽ�����ѯ���̣�����onFinished()���̣������û���ȡ����Ҫ����Ϣ
									EzServerClient.EasyXmlParse.EzMapserviceQuery.queryState = false;
									pMe.mapServiceQuery.onFinished();
								}
								return;
					...
					} else { //������Ϣ��ʵ200������״̬���ò�����ȷ��xml��Ϣ�������Ӳ�ѯerror,�����᷵�ظ��û���Ϣ(null)
	...
	EzServerClient.EasyXmlParse.EzMapserviceQuery = EzServerClient.EasyXmlParse.extend({
		//����
		options: {
			timeOut: null, 											//���õĳ�ʱʱ�� ��ʼĬ��ֵ��2000����
			tryTime: 3, 											//�����Ӳ�ѯ���Դ��� ��ʼĬ��ֵ��3
			failureOnError: false, 									//��ѯ�����ǲ������� ��ʼĬ��ֵ��false		
			itemCurrentCount: null, 								//��ǰδ�ɹ���ѯ���Ӳ�ѯ�������� ��ʼĬ��ֵ����
			queryFlag: 0 											//���õ�һ����ѯ�����ĳ�������
		},
	
		statics:{
			queryState: false, 										//��̬�����ԣ���ѯ������״̬��Ĭ����false,���ڲ�ѯ
			NextOrder: 0									//��̬�����ԣ����õĲ�ѯ��ȫ�ֱ�������¼�Ѿ����͵Ĳ�ѯ���Ӷ��� Ĭ�ϳ�ʼֵ��0
		},
	
		initialize:function(options){
			EzServerClient.setOptions(this, options);
	
			this.queryItem = new Array(); 							//��ѯ�Ӷ������� ��ʼĬ��ֵ����
			this.itemCount = null;									//��¼�Ӳ�ѯ�ĸ���
		},
		...
		doQuery:function(){
			...
			if (EzServerClient.EasyXmlParse.EzMapserviceQuery.queryState == false)
				...
				throw new Error("EzServerClient.EasyXmlParse.EzMapserviceQuery::doQuery��ѯ��û�в�ѯ���������ܽ����´β�ѯ");
			}
	...
	*/
	...
	EzServerClient.Control.NavButton = EzServerClient.Control.extend({
		...
		onAdd: function(map) {
			...
			this._panN_Button = this._createButton('北', className + '-panN control-navButton', panCircleContainer, this._panNorth, this);
	
			this._panW_Button = this._createButton('西', className + '-panW control-navButton', panCircleContainer, this._panWest, this);
	
			this._panS_Button = this._createButton('南', className + '-panS control-navButton', panCircleContainer, this._panSouth, this);
	
			this._panE_Button = this._createButton('东', className + '-panE control-navButton', panCircleContainer, this._panEast, this);
	...
	EzServerClient.Control.Zoomslider = EzServerClient.Control.extend({
		...
		onAdd: function(map) {
			...
			this._zoomInButton = this._createButton('放大', className + '-in', container, this._zoomIn, this);
			this._createSlider(className + '-slider', container, map);
			this._zoomOutButton = this._createButton('缩小', className + '-out', container, this._zoomOut, this);
	...
	*/
	...
	HeatMapOverlay = function(map,cfg){
		this._map=map;
		this._conf = cfg;
		this.heatmap=null;
		this.latlngs=[];
		this.initialize();
		window._heatmap=this;
	};
	HeatMapOverlay.prototype=new OverView();
	HeatMapOverlay.prototype={
		initialize:function(){
			...
			el.style.left = 0;
			el.style.border = 0;
			el.style.width = this._map.getLMap().getSize().x+"px";
			el.style.height = this._map.getLMap().getSize().y+"px";
			this._conf.container = el;
			...
			this._container=el;
			this._width=this._map.getLMap().getSize().x;
			this._height=this._map.getLMap().getSize().y;
			return el;
		},
		addTo:function(map){
			map.addLayer(this);
		...
		onAdd: function (map) {
			//this._map.getLMap().getPanes().markerPane.appendChild(this._container);
			this._map.getLMap().getPanes().overlayPane.appendChild(this._container);
			this.zoomHandler=this._map.addMapEventListener(EzEvent.MAP_ZOOMEND,this.draw);
			this.panHandler=this._map.addMapEventListener(EzEvent.MAP_PANEND,this.draw);
			this.resizeHandler=this._map.addMapEventListener(EzEvent.MAP_RESIZE,this.resize);
			if(!this.heatmap){
				this.heatmap=h337.create(this._conf);
			}
		},
		onRemove:function(){
			if(!this._map){return;}
			this._container.parentElement.removeChild(this._container);
			if(this.zoomHandler){this._map.removeMapEventListener(this.zoomHandler);this.zoomHandler = null;}
			if(this.panHandler){this._map.removeMapEventListener(this.panHandler);this.panHandler = null;}
			if(this.resizeHandler){this._map.removeMapEventListener(this.resizeHandler);this.resizeHandler = null;}
			this._map=null;
		},
		resize:function(){
			if (!_heatmap._map){ return; }
			var mapszie=_heatmap._map.getLMap().getSize();
			var w=mapszie.x;
			var h=mapszie.y;
			if (w == _heatmap._width && h == _heatmap._height){ return; }
			_heatmap._width = w;
			_heatmap._height = h;
			_heatmap.heatmap._renderer.setDimensions(w, h);
			_heatmap.draw();
		},
		setStartPoint:function(){
			if(!_heatmap._map){return;}
			//复位原点
			var tt=_heatmap._map.getLMap().getPanes().mapPane.style.cssText.split("(")[1];
			var te=tt.split(")")[0];
			var nleft=te.split(",")[0];
			var ntop=te.split(",")[1];
			var nll=parseInt(nleft.substr(0,nleft.length-2));
			var ntt=parseInt(ntop.substr(0,ntop.length-2));
			if(nll==0 && ntt==0){return;}
			_heatmap.heatmap._renderer.canvas.style.left=(0-nll)+'px';
			_heatmap.heatmap._renderer.canvas.style.top=(0-ntt)+'px';
		},
		draw: function(){
			_heatmap.setStartPoint();
			var cb = _heatmap._map.getBoundsLatLng();
			if (cb.equals(_heatmap.bounds)) {return;}
			_heatmap.bounds = cb;
			var ne=_heatmap._map.mapCoord2Container(cb._northEast);
			var sw=_heatmap._map.mapCoord2Container(cb._southWest);
			if(_heatmap.latlngs.length>0){
				_heatmap.heatmap.removeData();
				var d={
					max:_heatmap.heatmap._store.getData().max,
					data:[]
				};
				var n=_heatmap.latlngs.length;
				while(n--){
					var latlng=_heatmap.latlngs[n].latlng;
					if(!cb.contains(latlng)){continue;}
					var m=_heatmap.latlngToScreenPix(ne,sw,latlng);
					d.data.push({
						x:m.x,
						y:m.y,
						count:_heatmap.latlngs[n].c
					});
				}
				_heatmap.heatmap.setData(d);
			}
		},
		pixelTransform:function(p){
			var s=this._map.getLMap().getSize();
			var w = s.x;
			var	h = s.y;
			while(p.x < 0){ p.x+=w; }
			while(p.x > w){ p.x-=w; }
		...
		latlngToScreenPix:function(nePix,swPix,latlng){
			var pix=this._map.mapCoord2Container(latlng);
			var screenPix={x:pix.x-swPix.x,y:pix.y-nePix.y};
			return this.pixelTransform(screenPix);
		},
		setData: function(data){
			//data={max:max,data:data} data.data=[{lat:lat,lng:lng,count:count}]
			_heatmap.setStartPoint();
			var mapdata = {
				max: data.max,
				data: []
			...
			var n = d.length;
			this.latlngs = [];
			var cb = this._map.getBoundsLatLng();
			var nepix=this._map.mapCoord2Container(cb._northEast);
			var swpix=this._map.mapCoord2Container(cb._southWest);
			while(n--){
				var latlng = new EzServerClient.LatLng(d[n].lat, d[n].lng);
				this.latlngs.push({latlng: latlng, c: d[n].count});
				if(!cb.contains(latlng)){continue;}
				var m=this.latlngToScreenPix(nepix,swpix,latlng);
				mapdata.data.push({
					x:m.x,
					y:m.y,
					count:d[n].count
		...
		/*addDataPoint:function(lat, lng, count){
			var latlng = new EzServerClient.LatLng(lat, lng);
			var pix = this.pixelTransform(this._map.mapCoord2Container(latlng));
			this.heatmap._store.addDataPoint(pix.x, pix.y, count);
			this.latlngs.push({ latlng: latlng, c: count });
		},*/
		toggle:function(){
			if (this._conf.visible === true) {
				this._conf.visible = false
			} else {
				this._conf.visible = true
			}
			if (this._conf.visible) {
				this._conf.container.style.display = "block"
			} else {
				this._conf.container.style.display = "none"
			}
		}
	}
}(window, document));
...
EzServerClient.Draw.Polyline = EzServerClient.Draw.Feature.extend({
	...
	initialize: function (map, options) {
		...
		//this._isMeasure=options.isMeasure;
	...
	addHooks: function () {
		...
		if (this._map) {
			...
			//qianleyi
			this._tipMarkers = [];
			this._tipMarkerGrouop = new EzServerClient.LayerGroup();
			this._map.addLayer(this._tipMarkerGrouop);
	...
	removeHooks: function () {
		...
		//qianleyi
		this._map.removeLayer(this._tipMarkerGrouop);
		delete this._tipMarkerGroup;
		delete this._tipMarkers;
	...
	_vertexChanged: function (latlng, added) {
		...
		//qianleyi
		this._tipMarkers.push(this._createTipMarker(latlng));
	},
	 ...
	 * @private
	 */
	_createTipMarker: function(latlng) {
		var text = this._getTooltipText();
		if (text.subtext === '0 m') {
			iconSize = new EzServerClient.Point(30, 16);
			htmlText = '起点';
		} else {
			iconSize = new EzServerClient.Point(60, 16);
			htmlText = text.subtext;
		}
		var marker=null;
		if(this.options.isMeasure!=false){
			marker = new EzServerClient.Marker(latlng, {
				icon: new EzServerClient.DivIcon({
					iconSize: iconSize,
					iconAnchor: [-10,20],
					html: htmlText,
					className: 'EzServerClient-Tip-icon'
				}),
				zIndexOffset: this.options.zIndexOffset * 2
			});
			this._tipMarkerGrouop.addLayer(marker);
		}
		return marker;
	},
	...
	_updateTooltip: function (latLng) {
		...
		if (!this._errorShown) {
			this._tooltip.updateContent(text,this.options);
		}
	},
...
EzServerClient.Polyline.Measure = EzServerClient.Draw.Polyline.extend({
	addHooks: function() {
		EzServerClient.Draw.Polyline.prototype.addHooks.call(this);
		if (this._map) {
			this._markerGroup = new EzServerClient.LayerGroup();
			this._map.addLayer(this._markerGroup);
	...
	removeHooks: function () {
		...
		EzServerClient.Draw.Polyline.prototype.removeHooks.call(this);
	...
	_finishShape: function () {
		...
		if(this.options.isMeasure!=false){
			//qianleyi
			var marker = new EzServerClient.Marker(this._poly.getLatLngs()[this._poly.getLatLngs().length - 1],{
				icon: new EzServerClient.DivIcon({
					iconSize: new EzServerClient.Point(16,16),
					iconAnchor: [-10,38],
					className: 'EzServerClient-div-button'
				}),
				zIndexOffset: this.options.zIndexOffset * 2
			});
			this._tipMarkerGrouop.addLayer(marker);
	...
	_removeShape: function() {
		...
		this._tipMarkers.splice(0);
		this._tipMarkerGrouop.clearLayers();
	},
	...
	_getTooltipText: function() {
		var labelText = EzServerClient.Draw.Polyline.prototype._getTooltipText.call(this);
		if (!this._drawing) {
			labelText.text = '';
		}
		return labelText;
...
EzServerClient.Control.MeasureControl = EzServerClient.Control.extend({
	...
	onAdd: function(map) {
		...
		this._container = EzServerClient.DomUtil.create('div', 'leaflet-bar');

		this.handler = new EzServerClient.Polyline.Measure(map, this.options.handler);

		this.handler.on('enabled', function () {
			EzServerClient.DomUtil.addClass(this._container, 'enabled');
		}, this);

		this.handler.on('disabled', function () {
			EzServerClient.DomUtil.removeClass(this._container, 'enabled');
		}, this);

		var link = EzServerClient.DomUtil.create('a', className+'-measure', this._container);
		link.href = '#';
		link.title = EzServerClient.Control.MeasureControl.TITLE;

		EzServerClient.DomEvent
			.addListener(link, 'click', EzServerClient.DomEvent.stopPropagation)
			.addListener(link, 'click', EzServerClient.DomEvent.preventDefault)
